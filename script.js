...
let totalJob = document.getElementById('total-number');
...
allTabBtn.addEventListener('click', function(){
    ...
    // If allCardsList is empty, display the "no jobs available" card
    if (allCardsList.length === 0) {
        allCards.innerHTML = ''; // Clear existing content
        allCards.appendChild(createNoJobsCardElement());
    } else {
...
function listAllCards(){
    ...
    if (allCardsList.length === 0) {
        allCards.innerHTML = '';
        allCards.appendChild(createNoJobsCardElement());
        tabJob.innerText = ''; // Reset tabJob for 'All' tab if no jobs
    }
}  
listAllCards();
...
mainContainer.addEventListener('click', function(event){
    ...
    if(event.target.classList.contains('btn-interview')){
        ...
        if(allCardsList.length === 0){
            allCards.innerHTML = ''; // Clear all existing cards
            allCards.appendChild(createNoJobsCardElement());
            if (currentStatus === 'all') { // Only update tabJob if 'all' tab is active
                tabJob.innerText = '';
            }
        }
...
// Function to create and return the "No jobs available" card element
function createNoJobsCardElement() {
...
function renderInterviewCards(){
    ...
    if(interviewList.length === 0){
        filteredSection.appendChild(createNoJobsCardElement());
    }
    else{
        filteredSection.innerHTML = '';
        for(let interviewCard of interviewList){
            let div = document.createElement('div');
            div.className = 'card flex-row justify-between p-8 m-5 bg-slate-100 rounded-sm '
            div.innerHTML= `
